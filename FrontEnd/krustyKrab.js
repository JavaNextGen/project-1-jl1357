...
async function getReimbByStatus(id) {
    $("#employeeBody").empty();
    //we will send a fetch request to get our employee data
    //by default, fetch requests send GET requests
    //let response = await fetch(url + "employee");
    let response = await fetch(url + "reimbfilter/"+ id);
    //logging the response in the console just to see the response object
    console.log(response);
...
async function getuserReimb(id) {
    ...
    // console.log(data2);
    let response = await fetch(url + "reimbforuser/"+id);
    //logging the response in the console just to see the response object
    console.log(response);
...
async function loginFunction() {
    ...
    if(response.status === 202) {
        ...
        window.localStorage.setItem('data2', JSON.stringify(data2));
        ...
        //export {data2};
            if(data2.role==="FINANCE_MANAGER"){
                ...
                //document.getElementById("getEmployeeButton").innerHTML="ucant";
            }
